...
public:
	...
	Player::Player(int i) { // Player constructor
		...
		name = get_string();
        cout << "Ah! Welcome to SPC Adventure " << name << "! That is a cool name!";
		system("cls");
		wait(0.5);
		cout << "\nInitializing " << name << "'s attributes." << endl;
		level = 1;
		wait(0.5);
		cout << "Setting strength level";
		dot_dot_dot();
		strength = roll_attribute();
		wait(0.5);
		cout << "Setting stamina level";
		dot_dot_dot();
		stamina = roll_attribute();
		wait(0.5);
		cout << "Setting speed level";
		dot_dot_dot();
		...
		wait(1.5);
		system("cls");
		cout << endl << "Your character, " << name << " has been created!\n" << "You are all finished! Are your ready to start your adventure?\n" <<endl; // dialogue edit
	...
	void print_position() {
		cout << "Your Current Position is: (" << this->xPosition << ", " << this->yPosition << ")" << endl; // dialogue edit
	}
